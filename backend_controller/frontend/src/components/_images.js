// This file is used for webpack to recognize images and 
// make the browser also recognize images in the bundle file.

const IMAGES = {

    // Important Images
    GFVLuresLogo: require("./images/GVFLuresLogo.jpg").default,
    GFVLuresLogo2: require("./images/GVFLuresLogo2.jpg").default,
    Ourboat: require("./images/ourboat.jpg").default,
    Rich1: require("./images/Rich1.jpg").default,
    Rich2: require("./images/Rich2.jpg").default,
    RichnMe: require("./images/RichnMe.jpg").default,
    RichnMe2: require("./images/RichnMe2.jpg").default,
    TigerLures: require("./images/TigerLures.jpg").default,

    // Lakes on Computer
    Berryessa: require("./images/berryessa.jpg").default,
    Bluemesa: require("./images/bluemesa.jpg").default,
    Flaming: require("./images/flaminggorge.jpg").default,
    Hellhole: require("./images/hellhole.jpg").default,
    Newbullardsbar: require("./images/newbullardsbar.jpg").default,
    Newmelones: require("./images/newmelones.jpg").default,
    Pardee: require("./images/pardee.jpg").default,
    Pyramid: require("./images/pyramid.jpg").default,
    Stampede: require("./images/stampede.jpg").default,

    // Lakes on Mobile
    MobileBerryessa: require("./images/mobile-berryessa.jpg").default,
    
    // Lures on Lake Page
    LPLure1: require("./images/LPLure1.jpg").default,
    LPLure2: require("./images/LPLure2.jpg").default,
    LPLure3: require("./images/LPLure3.jpg").default,
    LPLure4: require("./images/LPLure4.jpg").default,
    LPLure5: require("./images/LPLure5.jpg").default,
    LPLure6: require("./images/LPLure6.jpg").default,
    LPLure7: require("./images/LPLure7.jpg").default,
    LPLure8: require("./images/LPLure8.jpg").default,

    // Fishing and People
    FishingP1: require("./images/FishingP1.jpg").default,
    FishingP2: require("./images/FishingP2.jpg").default,
    FishingP3: require("./images/FishingP3.jpg").default,
    FishingP4: require("./images/FishingP4.jpg").default,
    FishingP5: require("./images/FishingP5.jpg").default,
    FishingP6: require("./images/FishingP6.jpg").default,
    FishingP7: require("./images/FishingP7.jpg").default,
    FishingP8: require("./images/FishingP8.jpg").default,
    FishingP9: require("./images/FishingP9.jpg").default,
    FishingP10: require("./images/FishingP10.jpg").default,
    FishingP11: require("./images/FishingP11.jpg").default,

    // Fish and Lures
    Fish_Lures1: require("./images/Fish_Lures1.jpg").default,
    Fish_Lures2: require("./images/Fish_Lures2.jpg").default,
    Fish_Lures3: require("./images/Fish_Lures3.jpg").default,
    Fish_Lures4: require("./images/Fish_Lures4.jpg").default,
    Fish_Lures5: require("./images/Fish_Lures5.jpg").default,
    Fish_Lures6: require("./images/Fish_Lures6.jpg").default,
    Fish_Lures7: require("./images/Fish_Lures7.jpg").default,
    Fish_Lures8: require("./images/Fish_Lures8.jpg").default,
    Fish_Lures9: require("./images/Fish_Lures9.jpg").default,
    Fish_Lures10: require("./images/Fish_Lures10.jpg").default,

    // Slide Images
    Slide1: require("./images/Slide1.jpg").default,
    Slide2: require("./images/Slide2.jpg").default,
    Slide3: require("./images/Slide3.jpg").default,

    // Goofy
    GoofyImageOfMe: require("./images/GoofyImageOfMe.jpg").default
}

export default IMAGES;